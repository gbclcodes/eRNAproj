...
tfJSscoreVec[,2] <- factor(tfJSscoreVec[,2],levels=rev(c("MIIOocyte","X2cell","X4cell","X8cell","morula","ICM")),ordered=TRUE)
...
library(wesanderson)
library(dendextend)
pdf(file="/media/yuhua/yuhua_projects/enhProj/ENHData/enh_stage_group_files/GSR/tf_tf_simi_clustering_heatmap.pdf",width=20,height=4)
p <- pheatmap(t(plotMat),show_rownames=TRUE,scale="column",clustering_distance_cols="manhattan",cluster_rows=FALSE,cluster_cols=TRUE,fontsize=5,border_color="NA",color = colorRampPalette(c("#1E90FF","#FFFFF0","#FF00FF"))(100))
dev.off()

pdf(file="/media/yuhua/yuhua_projects/enhProj/ENHData/enh_stage_group_files/GSR/tf_tf_simi_clustering_tree.pdf",width=40,height=4)
p.dend <- as.dendrogram(p$tree_col)
p.dend <- reorder(p.dend,seq(1,2500000,10000),agglo.FUN="mean")
p.dend %>% set("branches_k_col", k = 5) %>% plot()
dev.off()

pdf(file="/media/yuhua/yuhua_projects/enhProj/ENHData/enh_stage_group_files/GSR/tf_tf_simi_clustering_heatmap.pdf",width=40,height=4)
...
write.table(cbind(plotMat[labels(p.dend),],maxStages)[seq(nrow(plotMat),1,-1),],file="/media/yuhua/yuhua_projects/enhProj/ENHData/enh_stage_group_files/GSR/tf_reg_strength_mat.txt",row.names=TRUE,col.names=TRUE)

tfCorMat <- cor(t(plotMat))
pdf(file="/media/yuhua/yuhua_projects/enhProj/ENHData/enh_stage_group_files/GSR/tf_tf_comodule_heatmap.pdf",width=40,height=40)
...
dev.off()
write.table(rev(labels(p.dend)),file="/media/yuhua/yuhua_projects/enhProj/ENHData/enh_stage_group_files/GSR/tfnames_in_modules.txt",row.names=FALSE,col.names=FALSE,quote=FALSE)
